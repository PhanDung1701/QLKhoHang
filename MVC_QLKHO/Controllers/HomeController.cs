...
namespace MVC_QLKHO.Controllers
{
    public class HomeController : BaseController
    {
        ...
        {
            using (var db = new dbQLKhoEntities())
            {
                var ob = db.VitriSuas.FirstOrDefault(q => q.ID_ViTri == id);

                if (ob == null)
                {
                    // If the record with the provided id is not found, return success (true)
                    return Json(true);
                }
                ...
                // Return true if rows were affected (record was successfully deleted), otherwise return false
                return Json(rowsAffected > 0);
            }
        }
